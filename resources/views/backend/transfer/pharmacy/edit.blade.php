...
<div class="page-body">
    <div class="container-fluid">
        <div class="page-title">
            <div class="row">
                <div class="col-6">
                    <h3>Transfer Update - Pharmacy</h3>
                </div>
                <div class="col-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">Transfer Update</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="col-sm-12">
                <div class="card">
                    <div class="card-header">
                        <h5>Update Pharmacy Transfer</h5><span>Update Pharmacy Transfer</span>
                    </div>
                    <div class="card-body">
                        <div class="card-wrapper">
                            <form class="row g-3" method="post" action="{{ route('pharmacy.transfer.update', $transfer->id) }}" id="transferForm">
                                @csrf
                                <div class="col-md-2">
                                    <label class="form-label req">Transfer Date</label>
                                    {{ html()->date($name = 'transfer_date', $value = $transfer->transfer_date->format('Y-m-d'))->class('form-control')->placeholder('Transfer Date')->required() }}
                                    @error('transfer_date')
                                ...
                                <div class="col-md-6">
                                    ...
                                    {{ html()->textarea($name = 'transfer_note', $value = $transfer->transfer_note)->class('form-control')->rows('3')->placeholder('Transfer Note') }}
                                </div>
                                <div class="row g-4 table-responsive">
                                    <div class="col text-end">
                                        <div class="btn-group">
                                            <button class="btn btn-primary" type="button" onclick="addTransferRow('pharmacy', 'transfer')">Add New Row</button>
                                        </div>
                                    </div>
                                    <div class="col-12">
                                        ...
                                        <table class="table table-bordered table-sm table-stripped mt-3">
                                            ...
                                            <tbody class="tblPharmacyTransferBody">
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        {{ html()->select('product_id[]', $products, $item->product_id)->class('border-0 select2 selPdctForTransfer')->attribute('data-category', 'pharmacy')->attribute('id', '')->placeholder('Select')->required() }}
                                                    </td>
                                                    <td>
                                                        <select class="form-control select2 selBatch" name="batch_number[]" required>
                                                            <option value="{{ $item->batch_number }}">{{ $item->batch_number }}</option>
                                                        </select>
                                                    </td>
                                                    <td><input type="number" name='qty[]' class="w-100 border-0 text-end" placeholder="0" min='1' step="1" value="{{ $item->qty }}" required /></td>
                                                </tr>
                                                @empty
                                                @endforelse
                                            </tbody>
