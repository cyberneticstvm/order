                        ...
                        <div class="card-wrapper">
                            <form class="row g-3" method="post" action="{{ route('report.daybook.fetch') }}">
                                ...
                                <div class="col-md-3">
                                    ...
                                    {{ html()->select($name = 'branch', $value = (in_array(Auth::user()->roles->first()->name, ['Administrator', 'CEO', 'Accounts'])) ? $branches->prepend('All', '0') : $branches, ($inputs[2]) ?? old('branch'))->class('form-control select2')->placeholder('Select') }}
                                    @error('branch')
