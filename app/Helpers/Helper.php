...
function getInventory($branch, $product, $category)
{
    ...
    if ($category == 'pharmacy') :
        ...
        if ($branch == 0) :
            $stock = DB::select("SELECT 'Main Stock' AS branch, tbl1.product_name, tbl1.purchasedQty, SUM(CASE WHEN t.from_branch_id = 0 AND t.deleted_at IS NULL AND t.transfer_status = 1 THEN td.qty ELSE 0 END) AS transferredQty, SUM(CASE WHEN dam.deleted_at IS NULL THEN dam.qty ELSE 0 END) AS damagedQty, tbl1.purchasedQty - (IFNULL(SUM(CASE WHEN t.from_branch_id = 0 AND t.deleted_at IS NULL THEN td.qty ELSE 0 END), 0) + IFNULL(SUM(CASE WHEN dam.deleted_at IS NULL THEN dam.qty ELSE 0 END), 0)) AS balanceQty FROM (SELECT pdct.id AS product_id, pdct.name AS product_name, SUM(pd.qty) AS purchasedQty FROM purchase_details pd LEFT JOIN products pdct ON pd.product_id = pdct.id WHERE IF(? > 0, pd.product_id = ?, 1) GROUP BY pdct.id, pdct.name) AS tbl1 LEFT JOIN transfer_details td ON tbl1.product_id = td.product_id LEFT JOIN transfers t ON t.id = td.transfer_id LEFT JOIN product_damages dam ON tbl1.product_id = dam.product_id GROUP BY product_name, purchasedQty", [$product, $product]);
        else :
            $branch = Branch::findOrFail($branch);
            $bname = $branch->name;
            $stock = DB::select("SELECT '$bname' AS branch, tbl1.product_name, tbl1.purchasedQty, tbl1.transferredQty, SUM(CASE WHEN o.branch_id = ? AND o.deleted_at IS NULL THEN od.qty ELSE 0 END) AS soldQty, SUM(CASE WHEN sr.returned_branch=? AND sr.deleted_at IS NULL THEN srd.returned_qty ELSE 0 END) AS returnedQty, (tbl1.purchasedQty + SUM(CASE WHEN sr.returned_branch=? AND sr.deleted_at IS NULL THEN srd.returned_qty ELSE 0 END)) - ((tbl1.transferredQty+IFNULL(SUM(CASE WHEN o.branch_id = ? AND o.deleted_at IS NULL THEN od.qty ELSE 0 END), 0) + IFNULL(SUM(CASE WHEN dam.deleted_at IS NULL AND dam.from_branch = ? THEN dam.qty ELSE 0 END)), 0)) AS balanceQty, SUM(CASE WHEN dam.deleted_at IS NULL AND dam.from_branch = ? THEN dam.qty ELSE 0 END) AS damagedQty FROM (SELECT pdct.id AS product_id, pdct.name AS product_name, SUM(CASE WHEN t.to_branch_id = ? THEN td.qty ELSE 0 END) AS purchasedQty, SUM(CASE WHEN t.from_branch_id = ? THEN td.qty ELSE 0 END) AS transferredQty FROM transfer_details td LEFT JOIN transfers t ON t.id = td.transfer_id LEFT JOIN products pdct ON td.product_id = pdct.id WHERE IF(? > 0, td.product_id = ?, 1) AND t.deleted_at IS NULL AND t.transfer_status = 1 GROUP BY pdct.id, pdct.name) AS tbl1 LEFT JOIN order_details od ON od.product_id = tbl1.product_id LEFT JOIN orders o ON o.id = od.order_id LEFT JOIN product_damages dam ON tbl1.product_id = dam.product_id LEFT JOIN sales_return_details srd ON tbl1.product_id = srd.product_id LEFT JOIN sales_returns sr ON sr.id = srd.return_id GROUP BY product_name, purchasedQty, transferredQty", [$branch->id, $branch->id, $branch->id, $branch->id, $branch->id, $branch->id, $branch->id, $branch->id, $product, $product]);
        endif;
    endif;
