...
{
    OrderDetail::selectRaw("IFNULL(COUNT(order_details.qty), 0) AS soldQty")->leftJoin('orders AS o', 'o.id', 'order_details.order_id')->whereBetween('o.created_at', [Carbon::parse($from_date)->startOfDay(), Carbon::parse($to_date)->endOfDay()])->when($product > 0, function ($q) use ($product) {
