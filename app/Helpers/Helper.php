...
function getInventory($branch, $product, $category)
{
    ...
    if ($category == 'pharmacy') :
        ...
        if ($branch == 0) :
            $stock = DB::select("SELECT 'Main Stock' AS branch, tbl1.product_name, tbl1.purchasedQty, SUM(CASE WHEN t.from_branch_id = 0 AND t.deleted_at IS NULL AND t.transfer_status = 1 THEN td.qty ELSE 0 END) AS transferredQty, tbl1.purchasedQty - SUM(CASE WHEN t.from_branch_id = 0 AND t.deleted_at IS NULL THEN td.qty ELSE 0 END) AS balanceQty FROM (SELECT pdct.id AS product_id, pdct.name AS product_name, SUM(pd.qty) AS purchasedQty FROM purchase_details pd LEFT JOIN products pdct ON pd.product_id = pdct.id WHERE IF(? > 0, pd.product_id = ?, 1) GROUP BY pdct.id, pdct.name) AS tbl1 LEFT JOIN transfer_details td ON tbl1.product_id = td.product_id LEFT JOIN transfers t ON t.id = td.transfer_id GROUP BY product_name, purchasedQty HAVING balanceQty > 0", [$product, $product]);
        else :
            $branch = Branch::findOrFail($branch);
            $bname = $branch->name;
