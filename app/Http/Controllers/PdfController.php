...
class PdfController extends Controller
{
    ...
    {
        $res = generateInvoiceForOrder($id);
        if ($res):
    ...
    public function exportOrderInvoice($id)
    {
        ...
        if ($order->invoice_number) :
            $oid = $order->id * 100;
            $qrcode = base64_encode(QrCode::format('svg')->size(75)->errorCorrection('H')->generate(domain_url($order->branch->id) . '/bill/details/' . $oid));
    ...
    {
        $order = Order::findOrFail(decrypt($id));
        $advance = $order->payments->whereIn('payment_type', ['advance', 'advance1'])->sum('amount');
    ...
    public function exportOrderPrescriptionForWa($id)
    {
        ...
        return $pdf->stream($spectacle->id . '.pdf');
    }
    ...
    public function exportPurchase(string $id)
    {
