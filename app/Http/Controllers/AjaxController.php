...
class AjaxController extends Controller
{
    ...
    public function getProductsByCategory($category, $type, $product = 0)
    {
        ...
        if (($category == 'frame' || $category == 'solution') && $type == 'order' && $product == 0 && Session::get('branch') != 1):
    ...
    public function getOrderData()
    {
        ...
        })->select(DB::raw("LAST_DAY(CURRENT_DATE) + INTERVAL 1 DAY - INTERVAL months.id MONTH AS date, COUNT(CASE WHEN o.deleted_at IS NOT NULL THEN o.id END) AS order_count, CONCAT_WS('/', DATE_FORMAT(LAST_DAY(CURRENT_DATE) + INTERVAL 1 DAY - INTERVAL months.id MONTH, '%b'), DATE_FORMAT(LAST_DAY(CURRENT_DATE) + INTERVAL 1 DAY - INTERVAL months.id MONTH, '%y')) AS month"))->groupBy('date', 'months.id')->orderByDesc('date')->get();
    ...
    public function getOrderComparisonData($bid)
    {
        ...
        })->select(DB::raw("LAST_DAY(CURRENT_DATE) + INTERVAL 1 DAY - INTERVAL months.id MONTH AS date, COUNT(CASE WHEN o.deleted_at IS NOT NULL THEN o.id END) AS order_count, COUNT(CASE WHEN order_status = 'delivered' THEN o.id END) AS dcount, COUNT(CASE WHEN o.deleted_at IS NOT NULL THEN o.id END) - COUNT(CASE WHEN order_status = 'delivered' THEN o.id END) AS bcount, CONCAT_WS('/', DATE_FORMAT(LAST_DAY(CURRENT_DATE) + INTERVAL 1 DAY - INTERVAL months.id MONTH, '%b'), DATE_FORMAT(LAST_DAY(CURRENT_DATE) + INTERVAL 1 DAY - INTERVAL months.id MONTH, '%y')) AS month"))->groupBy('date', 'months.id')->orderByDesc('date')->get();
        return json_encode([
            'ord' => $orders,
        ]);
    }
