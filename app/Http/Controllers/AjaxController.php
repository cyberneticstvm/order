...
class AjaxController extends Controller
{
    ...
    public function generatePaymentQr(Request $request)
    {
        ...
        $qr = base64_encode(QrCode::format('svg')->size(75)->backgroundColor(50, 205, 50)->errorCorrection('H')->generate('upi://pay?pa=' . $request->mobile . '@upi&pn=' . $pn . '&tn=' . $tn . '&am=' . ceil($am) . '&cu=INR'));
        return response()->json([
            'qrCode' => $qr,
        ]);
    }
