...
class AjaxController extends Controller
{
    ...
    public function content($branch, $oid)
    {
        ...
        $existing = OfferProduct::with('product')
    ...
    public function saveProductForOffer(Request $request)
    {
        ...
        $conflict = DB::transaction(function () use ($offer, $product) {
            ...
            if ($offer->isLensDiscount()) {
                $message = $this->findLensOfferConflict($offer, $product->id);
                if ($message) {
                    return $message;
                }
            }
        ...
        return response()->json([
            'msg' => $offer->isLensDiscount() ? 'Lens added to the collection offer successfully!' : 'Product added successfully!',
            'type' => 'success',
    ...
    private function findLensOfferConflict(OfferCategory $offer, int $lensId): ?string
    {
    ...
    {
        $products = Product::where('category', $category)->selectRaw("id, CONCAT_WS('-', name, code, tax_percentage) AS name")->orderBy('name')->get();
            ...
            if ($products):
                ...
                foreach ($products->where("balanceQty", ">", 0) as $key => $item):
                    ...
                    if ($purchase):
                        ...
                        $data .= "<input type='hidden' name='supplier_ids[]' value='" . $purchase?->purchase?->supplier?->id . "' />";
                        $data .= "<input type='hidden' name='product_ids[]' value='" . $item->id . "' />";
                        $data .= "<tr>";
                        $data .= "<td>" . $slno . "</td>";
                        $data .= "<td>" . $purchase?->purchase?->purchase_number ?? '' . "</td>";
                        $data .= "<td>" . $purchase?->purchase?->supplier?->name ?? '' . "</td>";
                        ...
                        $data .= "<td><input type='text' class='form-control text-end' name='prices[]' value='" . $purchase?->unit_price_purchase . "' readonly /></td>";
