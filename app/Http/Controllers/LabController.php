...
class LabController extends Controller
{
    ...
    public function assignOrders()
    {
        ...
        $ord = OrderDetail::leftJoin("lab_orders as lo", "lo.order_detail_id", "order_details.id")->leftJoin('orders as o', 'o.id', 'order_details.order_id')->selectRaw("order_details.*, lo.lab_id")->where('order_details.fitting', 1)->when(!in_array(Auth::user()->roles->first()->name, array('Administrator', 'CEO')), function ($q) {
            return $q->where('o.branch_id', Session::get('branch'));
        })->whereNotIn('o.order_status', ['delivered', 'ready-for-delivery'])->whereNull('o.deleted_at')->whereNull("lo.lab_id");
    ...
    public function labOrdersUpdateStatus(Request $request)
    {
        ...
        DB::transaction(function () use ($request) {
            ...
            foreach ($request->chkItem as $key => $item) :
                ...
                if ($request->status == 'sent-to-lab') :
                    $action  = 'Order has transferred to ' . Branch::where('id', ($request->lab_id) ?? $lab->getOriginal('lab_id'))->first()?->name ?? 'Main Branch';
                    $action .= ' - ' . strtoupper($odetail->eye);
                endif;
