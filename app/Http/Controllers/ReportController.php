...
class ReportController extends Controller
{
    ...
    function hsn_product_fetch(Request $request)
    {
        ...
        $branches = $this->branches;
        $hsn = $request->hsn;
        $records = DB::table("order_details")->leftJoin('orders AS o', 'o.id', 'order_details.order_id')->selectRaw("order_details.eye AS hsn, SUM(order_details.qty) AS qty, SUM(order_details.total) AS total")->whereNull("o.deleted_at")->whereBetween("o.created_at", [Carbon::parse($request->from_date)->startOfDay(), Carbon::parse($request->to_date)->endOfDay()])->when($request->branch > 0, function ($q) use ($request) {
            return $q->where('o.branch_id', $request->branch);
        })->when($hsn == 'frame', function ($q) use ($request) {
            return $q->whereIn('order_details.eye', ['frame']);
        })->when($hsn == 'lens', function ($q) use ($request) {
            return $q->whereIn('order_details.eye', ['re', 'le']);
