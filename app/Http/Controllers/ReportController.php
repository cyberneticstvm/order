...
class ReportController extends Controller
{
    ...
    public function fetchProductTransfer(Request $request)
    {
        ...
        $data = Transfer::leftJoin('transfer_details as td', 'td.transfer_id', 'transfers.id')->selectRaw("transfers.*, sum(td.qty) as pcount")->whereBetween('transfers.created_at', [Carbon::parse($request->from_date)->startOfDay(), Carbon::parse($request->to_date)->endOfDay()])->when($request->branch > 0, function ($q) use ($request) {
