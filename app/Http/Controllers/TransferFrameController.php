...
class TransferFrameController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->middleware(function ($request, $next) {
            ...
            $this->branches = Branch::selectRaw("id, name")->where('id', Session::get('branch'))->when(Auth::user()->roles->first()->name == 'Administrator' || Auth::user()->roles->first()->name == 'CEO', function ($q) use ($brs) {
                return $q->union($brs);
            })->orderBy('id')->pluck('name', 'id');

            $this->tobranches = Branch::selectRaw("id, name")->where('id', '<>', Session::get('branch'))->when(Auth::user()->roles->first()->name == 'Administrator' || Auth::user()->roles->first()->name == 'CEO', function ($q) use ($brs) {
                return $q->union($brs);
            })->orderBy('id')->pluck('name', 'id');
