$(function () {
    ...
    $(document).on('change', '#orderForm[data-lens-offers="1"] .lensOfferProduct, #orderForm[data-lens-offers="1"] .lensOfferFrame, #orderForm[data-lens-offers="1"] select[data-category="frame"]', scheduleLensOfferResolution);
    ...
    $.ajax({
        ...
        success: function (res) {
            if (category == 'pharmacy') {
                ...
                $(".tblPharmacyTransferBody").append(`<tr><td class="text-center"><a href="javascript:void(0)" class="dltRow"><i class="fa fa-trash text-danger"></i></a></td><td><select class="form-control select2 selPdctForTransfer" name="product_id[]" data-category="${category}" required><option></option></select></td><td class='qtyAvailable text-end'>0</td><td><input type="number" name='qty[]' class="w-100 border-0 qtyMax text-end pQty" placeholder="0" min='1' step="1" required /></td></tr>`);
            }
            var xdata = $.map(res.products, function (obj) {
                obj.text = obj.name || obj.id;
                return obj;
